...
- 랜덤화가 의미있으려면, 노드의 순서를 임의로 바꿨을 때 트리의 높이의 기대치가 $O(N)$ 보다 작아야 한다.

- $N$개의 원소를 갖는 트립에서 어떤 원소 $x$를 찾으려고 할 때 방문해야 하는 노드의 수의 기대치가 $O(lgN)$임을 증명
...
- 상황 : $root$를 루트로 하는 트립에 새 노드 $node$ 를 삽입
- 체크 : $root$ 와 $node$ 의 우선순위
  - $root$ 의 우선순위가 높으면 $node$ 는 $root$ 아래로 들어가야한다.
    - 왼쪽/오른쪽 서브트리인지는 두 노드의 원소 값을 비교해 결정
    - 재귀호출을 통해 해당 서브트리에 $node$ 를 삽입
  - $node$ 의 우선순위가 $root$ 보다 높은 경우에는 $node$ 가 기존에 있던 루트 $root$ 를 밀어내고 루트가 되어야 한다.
    - 기존의 트리를 $node$ 가 가진 원소를 기준으로 **쪼개는** 것
    - 트리를 쪼개서 기준보다 작은 원소만을 갖는 서브트리 하나, 큰 원소만을 갖는 서브트리 하나를 만들어 $node$ 양 쪽에 서브트리로 두면 된다.
...
- 왼쪽 서브트리의 크기가 $l$ 이라고 할 때, 다음과 같은 3가지 경우 존재
  - $k \leq l$ : $k$ 번째 노드는 왼쪽 서브트리에 속해 있다.
  - $k = l + 1 $ : 루트가 $k$ 번째 노드
  - $k > l + 1 $ : $k$ 번째 노드는 오른쪽 서브트리에서 $k-l-1$ 번째 노드
- 시간복잡도는 트리의 높이에 비례하므로 균형잡힌 이진 검색 트리에서 $O(lgN)$
...
- $A[0 \cdots i-1]$ 이 정렬된 배열일 때, $A[i]$ 를 적절한 위치를 만날 때까지 왼쪽으로 한 칸씩 움직인다.
...
- 테스트 케이스 수 $C(1 \leq C \leq50)$ 
- 원 배열의 길이 $N(1 \leq N \leq 50000)$
- $N$ 개의 정수로 시작시에 $A$ 의 각 위치에 있던 값들이 움직이 칸수가 주어짐
- $A$ 는 1부터 $N$ 까지의 정수를 한 번씩 포함
...
- $A[4]$ 는 3칸 왼쪽으로 움직임
  ...
  &#8594; 1~5 범위의 숫자 중에서 $A[4]$ 보다 큰 숫자가 3개 있다는 뜻

-  문제는 $A[i]$ 에 들어갈 수 있는 숫자들의 집합을 어떻게 저장할 것이냐는 것이다.

  &#8594; 후보들의 잡합 중 $k$ 번째 원소가 무엇인지를 찾고, 삭제하는 작업을 빠르게 수행할 수 있어야 한다.

- `kth()` 와 `erase()` 함수의 수행시간이 모두 $O(lgN)$ 이므로 `solve()`의 전체 시간 복잡도는 $O(NlgN)$ 이다.
